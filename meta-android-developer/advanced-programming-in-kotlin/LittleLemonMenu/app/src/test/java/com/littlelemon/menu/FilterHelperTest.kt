...
class FilterHelperTest {
    ...
    fun filterProducts_filterTypeDessert_croissantReturned() {
        ...
        // Assert
        assertEquals(1, result.size)                 // only one product should remain
        assertEquals("Croissant", result[0].title)   // it should be "Croissant"
        assertEquals("Dessert", result[0].category) // category should be Dessert
    }
}
